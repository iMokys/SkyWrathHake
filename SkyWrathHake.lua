...
function SkyWrathHake.OnDraw()
    ...
    local enemyClosest = SkyWrathHake.getComboTarget(myHero)
    if Menu.IsEnabled(SkyWrathHake.IsTargetParticleEnabled) then
        if not particleEnemy or (not enemyClosest and targetParticle ~= 0) or enemy ~= particleEnemy then
            Particle.Destroy(targetParticle)
            targetParticle = 0
            particleEnemy = enemy
        else
            if targetParticle == 0 and enemyClosest then
...
function SkyWrathHake.getComboTarget(myHero)

    if not myHero then return end

    local targetingRange = Menu.GetValue(SkyWrathHake.enemyInRange)
...
function SkyWrathHake.PrayToDog()
    ...
    if not enemy or not enemyClosest then
        return
    end
